...
def cleaning_data(X, delete_over=10, col_to_change_to_null=0.1):
    ...
    columns_to_delete = [
        col for col in categorical_variables if len(list(X[col].unique())) > delete_over
    ]
    ...
    columnas_modificar_por_1 = [
        col
        for col in X.columns
        if X[col].isnull().sum() > int(X.shape[0] * col_to_change_to_null)
    ]
